...
namespace eCommerce.Web.Areas.Dashboard.Controllers
{
    public class ProductsController : DashboardBaseController
    {
        ...
        public async Task<JsonResult> Action(FormCollection formCollection)
        {
            ...
            try
            {
                ...
                if (model.ProductID > 0)
                {
                    ...
                    {
                        newProductSpecification = model.ProductSpecifications.Select(x => new ProductSpecification() { ProductRecordID = currentLanguageRecord.ID, FeatureValueID = x.FeatureValueID }).ToList();
                    }
                ...
                else
                {
                    ...
                    if (model.ProductSpecifications != null)
                    {
                        ...
                        currentLanguageRecord.ProductSpecifications.AddRange(model.ProductSpecifications.Select(x => new ProductSpecification() { ProductRecordID = product.ID, FeatureValueID = x.FeatureValueID }));
                    }
                    if (!ProductsService.Instance.SaveProductRecord(currentLanguageRecord))
                    {
                    ...
                    //Whatsapp notification to users
                    using (var userManager = HttpContext.GetOwinContext().GetUserManager<eCommerceUserManager>())
                    {
                        foreach (var user in userManager.Users.Where(_ => _.isActive == true && _.PhoneNumber != null))
                        {
                            var CategoryName = CategoriesService.Instance.GetCategoryByID(product.CategoryID).SanitizedName;
                            var productLink = Url.ProductDetails(CategoryName, product.ID, productType.ID);
                            var baseUrl = HttpContext.Request.Url.GetLeftPart(UriPartial.Authority);
                            var fullLink = string.Format("{0}{1}", baseUrl, productLink);
                            await WhatsAppService.Instance.SendAsync(user.PhoneNumber, EmailTextHelpers.WhatsApp_NewProduct(fullLink));
                        }
                    }
            ...
            foreach (string key in formCollection)
            {
                if (key.Contains("specification"))
                {
                    ...
                    if (!string.IsNullOrEmpty(value))
                    {
                        ...
                        {
                            model.ProductSpecifications.Add(new ProductSpecification() { FeatureValueID = int.Parse(specificationValue) });
                        }
                    }
                }
            }
        ...
        [HttpPost]
        public JsonResult AsyncProducts()
        {
            JsonResult result = new JsonResult();
            try
            {
                var resultAsync = ProductsService.Instance.SyncProducts();


                result.Data = new { Success = resultAsync, Message = resultAsync ? string.Empty : "Dashboard.Products.UnableToSyncProducts".LocalizedString() };
            }
            catch (Exception ex)
            {
                result.Data = new { Success = false, Message = ex.Message };
