...
namespace eCommerce.Web.Areas.API.Controllers
{
    public class OrdersController : APIBaseController
    {
        ...
        public async Task<APIResult> PlaceOrderViaCashOnDelivery(PlaceOrderModel model)
        {
            ...
            {
                var cartProducts = new List<ProductType>();
                ...
                if (SessionHelper.CartItems != null && SessionHelper.CartItems.Count > 0)
                {
                    ...
                    {
                        cartProducts = ProductsService.Instance.GetProductTypesByIDs(productIDs);
                    }
                }
                ...
                if (cartProducts != null && cartProducts.Count > 0)
                {
                    ...
                    if (OrdersService.Instance.SaveOrder(newOrder))
                    {
                        ...
                        if (!newOrder.IsGuestOrder)
                        {
                            ...
                            await UserManager.SendEmailAsync(newOrder.CustomerID, EmailTextHelpers.OrderPlacedEmailSubject(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId), EmailTextHelpers.OrderPlacedEmailBody(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId, this.Url.Link("OrderTrack", new { orderID = newOrder.ID })));

                            //send order placed notification email to admin emails
                            await new EmailService().SendToEmailAsync(ConfigurationsHelper.FromEmailAddressName, ConfigurationsHelper.FromEmailAddress, ConfigurationsHelper.AdminEmailAddress, EmailTextHelpers.OrderPlacedEmailSubject_Admin(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId), EmailTextHelpers.OrderPlacedEmailBody_Admin(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId, this.Url.Link("EntityDetails", new { controller = "Orders", area = "Dashboard", ID = newOrder.ID })));
                        }
