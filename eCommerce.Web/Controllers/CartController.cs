    ...
    public class CartController : PublicBaseController
    {
        ...
        public ActionResult UpdateCart(FormCollection formCollection)
        {
            ...
            if (SessionHelper.CartItems != null)
            {
                ...
                {
                    model.Products = ProductsService.Instance.GetProductTypesByIDs(productIDs);
                }
                ...
                if (model.Products != null && model.Products.Count > 0)
                {
                    foreach (var product in model.Products)
                    {
                        ...
                        var quantity = cartItemsUpdate.CartItems.FirstOrDefault(x => x.ItemID == product.ID).Quantity;
                        ...
                        {
                            SessionHelper.CartItems.Add(new CartItem() { ItemID = product.ID, Price = productPrice, Quantity = product.StockQuantity });
                        }
                    }
                }
        ...
        public JsonResult AddItemToCart(int itemID, int quantity = 1)
        {
            ...
            var product = ProductsService.Instance.GetProducTypeByID(itemID);
