...
namespace eCommerce.Web.Controllers
{
    public class HomeController : PublicBaseController
    {
        ...
        public ActionResult Discount(string category, string q, decimal? from, decimal? to, string sortby, int? pageNo, int? recordSize)
        {
            recordSize = recordSize ?? (int)RecordSizeEnums.Size20;
            ...
            model.Products = ProductsService.Instance.SearchProducts(selectedCategoryIDs, model.SearchTerm, model.PriceFrom, model.PriceTo, model.SortBy, pageNo, recordSize.Value, activeOnly: true, out int count, stockCheckCount: null,isDicount: true);
        ...
        public async Task<JsonResult> SubmitContactForm(string subject, string name, string email, string message)
        {
            ...
            try
            {
                ...
                await new EmailService()
                                 .SendToEmailAsync(ConfigurationsHelper.FromEmailAddressName,
                                                   ConfigurationsHelper.FromEmailAddress,
