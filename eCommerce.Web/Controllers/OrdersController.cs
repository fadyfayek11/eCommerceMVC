...
namespace eCommerce.Web.Controllers
{
    ...
    public class OrdersController : PublicBaseController
    {
        ...
        public async Task<JsonResult> PlaceOrder(PlaceOrderCrediCardViewModel model)
        {
            ...
            if (model != null && ModelState.IsValid)
            {
                if (SessionHelper.CartItems != null && SessionHelper.CartItems.Count > 0)
                {
                    ...
                    {
                        model.Products = ProductsService.Instance.GetProductTypesByIDs(model.ProductIDs);
                    }
                }
                ...
                if (SessionHelper.CartItems != null && SessionHelper.CartItems.Count > 0 && model.Products != null && model.Products.Count > 0)
                {
                    ...
                    newOrder.OrderId = ((DateTime.Now.Date.Ticks) % rnd.Next()).ToString();
                    ...
                    if (authorizeNetResponse.Success)
                    {
                        ...
                        if (OrdersService.Instance.SaveOrder(newOrder))
                        {
                            ...
                            if (!newOrder.IsGuestOrder)
                            {
                                ...
                                await UserManager.SendEmailAsync(newOrder.CustomerID, EmailTextHelpers.OrderPlacedEmailSubject(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId), EmailTextHelpers.OrderPlacedEmailBody(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId, Url.Action("Tracking", "Orders", new { orderID = newOrder.OrderId }, protocol: Request.Url.Scheme)));

                                //send order placed notification email to admin emails
                                await new EmailService().SendToEmailAsync(ConfigurationsHelper.FromEmailAddressName, ConfigurationsHelper.FromEmailAddress, ConfigurationsHelper.AdminEmailAddress, EmailTextHelpers.OrderPlacedEmailSubject_Admin(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId), EmailTextHelpers.OrderPlacedEmailBody_Admin(AppDataHelper.CurrentLanguage.ID, newOrder.OrderId, Url.Action("Details", "Orders", new { area = "Dashboard", ID = newOrder.OrderId }, protocol: Request.Url.Scheme)));
                            }
            ...
            if (model != null && ModelState.IsValid)
            {
                ...
                if (SessionHelper.CartItems != null && SessionHelper.CartItems.Count > 0 && model.Products != null && model.Products.Count > 0)
                {
                    ...
                    foreach (var product in SessionHelper.CartItems.Select(x => model.Products.FirstOrDefault(y => y.ID == x.ItemID)))
                    {
                        ...
                        if (SessionHelper.CartItems.FirstOrDefault(x => x.ItemID == product.ID).Quantity > product.StockQuantity) 
                        {
                            jsonResult.Data = new { Success = false, Message = "PP.Shopping.ProductNotAvailableInSpecifiedQuantity".LocalizedString() };
                            return jsonResult;
                        }
        ...
        public ActionResult Tracking(string orderId, bool orderPlaced = false)
        {
            ...
            {
                model.Order = OrdersService.Instance.GetOrderByOrderId(orderId);
            }
        ...
        public ActionResult PrintInvoice(string orderId)
        {
            PrintInvoiceViewModel model = new PrintInvoiceViewModel
            {
                ...
                Order = OrdersService.Instance.GetOrderByOrderId(orderId)
            };
