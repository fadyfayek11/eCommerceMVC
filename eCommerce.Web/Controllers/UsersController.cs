...
namespace eCommerce.Web.Controllers
{
    ...
    public class UsersController : PublicBaseController
    {
        ...
        public async Task<JsonResult> Register(RegisterViewModel model)
        {
            ...
            var user = new eCommerceUser { PhoneNumber=model.PhoneNumber,ZipCode="00970", Address=model.Address ,Country = "فلسطين",City=model.City,isActive = false,FullName = model.FullName, UserName = model.Username, Email = model.Email, RegisteredOn = DateTime.Now };
            ...
            if (result.Succeeded)
            {
                ...
                await new EmailService().SendToEmailAsync(ConfigurationsHelper.FromEmailAddressName, ConfigurationsHelper.FromEmailAddress, ConfigurationsHelper.AdminEmailAddress,
                    EmailTextHelpers.AccountRegisterEmailSubject_Admin(AppDataHelper.CurrentLanguage.ID, user.UserName),
                    EmailTextHelpers.AccountRegisterEmailBody_Admin(AppDataHelper.CurrentLanguage.ID, user.UserName));
        ...
        public async Task<JsonResult> Login(LoginViewModel model)
        {
            ...
            var userIsActive = SignInManager.UserManager.FindByName(model.Username);
        ...
        [HttpGet]
        [AllowAnonymous]
        public ActionResult ResetPassword(string code, string userId,bool adminAction = false,bool isPartial = false)
        {
            ...
            if (adminAction && User.IsInRole("Administrator"))
            {
            ...
            if (isPartial)
            {
                return string.IsNullOrEmpty(code) || string.IsNullOrEmpty(userId) ? PartialView("Error") : PartialView("_resetPassword",model);
            }
        ...
        public async Task<JsonResult> UpdateProfile(UpdateProfileDetailsViewModel model)
        {
            ...
            if (User.IsInRole("Guest"))
            {
                jResult.Data = new { Success = false, Message = "UnAutharized" };
                return jResult;
            }
